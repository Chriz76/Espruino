...
bool dfu_enter_check(void) {
  ...
  bool dfu_start;
#ifdef BTN1_PININDEX
  dfu_start = (get_btn1_state() || get_charging_state());
#else
  dfu_start = dfuIsColdBoot; // if no button, enter bootloader if it's a cold boot, then exit after a few seconds
    ...
    if (dfu_start) {
      ...
      int count = 20;
#ifdef BANGLEJS_F18
      while ((get_btn1_state() || get_charging_state())&& --count) {
...
#ifdef BUTTONPRESS_TO_REBOOT_BOOTLOADER
  if (get_btn1_state() || get_charging_state()) rebootCounter++;
...
int main(void)
{
    ...
    print_fw_version();
#ifdef ESPR_BOOTLOADER_SPIFLASH
    if (!get_btn1_state() && !get_charging_state()) flashCheckAndRun();
#endif
#ifdef BANGLEJS
